...
class EVAgent(Agent):
    ...
    def initialise_locs(self):
        ...
        if len(self.model.POIs) > 3:
            ...
            self.locations = {
                "home": (rand_locs.iloc[0]["poi_x_km"], rand_locs.iloc[0]["poi_y_km"]),
            ...
            self.locations = {
                "home": (np.random.random() * self.model.width, np.random.random() * self.model.height),
                "work": (np.random.random() * self.model.width, np.random.random() * self.model.height),
                "random": (np.random.random() * self.model.width, np.random.random() * self.model.height),
            }
        # get from POI data
    ...
    def move(self):
        ...
        self.charge -= (
            self.dist_moved / self.efficiency_rating
        )  # moving uses charge at rate = discharge_rate  # change to distance travelled
    ...
    def update_possible_locations(self):
        ...
        if self.last_location == "random":
            ...
            if len(self.model.POIs) > 3:
                ...
                self.locations["random"] = (
                    np.random.random() * self.model.width,
    ...
    def step(self):
        ...
        if self.wait > 0:  # if still waiting then continue to wait
            ...
            if not self.moving:  # if not currently moving then choose new location and start moving
                ...
                self.charge_pcnt <= self.go_to_charge_pcnt
            ):  # if charge getting low, head straight to the nearest charge point
                self.find_closest_charge()
            self.moving = True
            self.charging = False
            self.move()
